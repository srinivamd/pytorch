...
class FlatParamHandle:
    ...
    def __init__(
        ...
        *,
        ...
        self._use_fake_all_gather = (
            os.environ.get(_FSDP_USE_FAKE_ALL_GATHER, "") == "1"
        )
        self._use_fake_reduce = (
            os.environ.get(_FSDP_USE_FAKE_REDUCE, "") == "1"
        )
    ...
    def _all_gather_flat_param(
        ...
        if sharded_flat_param.is_cpu:  # type: ignore[attr-defined]
            tensor_list = list(
                torch.chunk(
                    padded_unsharded_flat_param, dist.get_world_size(pg)
                )
            )
            work = dist.all_gather(
                tensor_list, sharded_flat_param, group=pg
            )
        else:
    ...
    def unshard_grad(self):
        ...
        pg = self._fake_process_group if self._use_fake_all_gather else self.process_group
        dist.all_gather_into_tensor(
            padded_unsharded_grad, sharded_grad, pg
        )
        unsharded_size = self.flat_param._unpadded_unsharded_size
